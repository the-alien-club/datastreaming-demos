 ...
 *
 * Adds records (OpenAIRE ids or DOIs) to the project's corpus.
 *
 * Body (addToCorpusSchema):
 *   ids    — string[] (1–5000 OpenAIRE ids and/or DOIs)
 *   reason — string   (recorded as the version note)
 *
 * Returns the new corpus snapshot with delta counters, plus `pending` (how many
 * added docs are still resolving in the background) and `unresolved` (DOIs the
 * Graph did not recognise). The add is instant — ids are stubbed synchronously
 * and their OpenAIRE metadata is resolved out-of-band by the resolver.
 *
...
export const POST = withAuth(async (req, user, bouncer, ctx: RouteCtx) => {
  ...
  const result = await CorpusService.addIds(project, user, parsed)
