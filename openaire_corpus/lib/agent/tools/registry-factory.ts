 ...
 * when `OPENAIRE_MCP_URL` / `OPENAIRE_MCP_TOKEN` are present in the environment,
 * the OpenAIRE MCP server entry. If the MCP env vars are absent (common in local
 * dev without a live endpoint) the registry still works — corpus, memory, and
 * ingest tools remain functional; the agent just has no OpenAIRE search
...
export async function buildTurnScopedRegistry(signal?: AbortSignal) {
  // MCP server is optional: if OPENAIRE_MCP_URL / OPENAIRE_MCP_TOKEN are absent —
  // or the session handshake fails (server down) — the app-defined corpus/memory/
  // ingest tools still work; the agent just has no OpenAIRE search capability for
  ...
  try {
    ...
    // The OpenAIRE MCP (mcp-base) runs stateless (no session). We still run the
    // `initialize` handshake for forward-compat with a stateful server: if it
    // returns a session id we thread it back as a header so the chat-sdk's
    // (session-blind) client echoes it on every tools/list + tools/call; if it
    // returns null we omit the header entirely. See lib/mcp/session.ts.
    const sessionId = await openMcpSession(
      mcpEnv.OPENAIRE_MCP_URL,
      mcpEnv.OPENAIRE_MCP_TOKEN,
      signal,
    )
    const headers: Record<string, string> = {
      Authorization: `Bearer ${mcpEnv.OPENAIRE_MCP_TOKEN}`,
    }
    if (sessionId) headers["Mcp-Session-Id"] = sessionId
    mcpServers = [{ name: "openaire", url: mcpEnv.OPENAIRE_MCP_URL, headers }]
  } catch (err) {
    console.warn(
      "[registry-factory] OpenAIRE MCP unavailable — agent has no OpenAIRE search " +
