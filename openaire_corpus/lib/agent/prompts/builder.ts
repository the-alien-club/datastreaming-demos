  ...
  private static async loadCorpusSnapshot(projectId: string) {
    ...
    const [headVersion, total, typeRows, langRows, yearRows] = await Promise.all([
      ...
      prisma.document.groupBy({
        by: ["instanceType"],
        where: { ...memberOf, instanceType: { not: null } },
        _count: true,
      }),
      prisma.document.groupBy({
        by: ["lang"],
    ...
    for (const r of typeRows) if (r.instanceType) type[r.instanceType] = r._count
    const lang: Record<string, number> = {}
    for (const r of langRows) if (r.lang) lang[r.lang] = r._count
    const period: Record<string, number> = {}
    for (const r of yearRows) {
      if (r.year === null) continue
      const dec = `${Math.floor(r.year / 10) * 10}s`
      period[dec] = (period[dec] ?? 0) + r._count
    }
