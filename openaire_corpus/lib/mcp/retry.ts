// lib/mcp/retry.ts
// Exponential-backoff retry + bounded-concurrency helpers for OpenAIRE MCP calls.
// No external dependencies — plain async/await only.

import { McpAuthError, McpNotFoundError, McpRateLimitError } from "./errors"
import {
...
export interface RetryOptions {
   ...
   * Defaults to the OpenAIRE MCP classification (McpAuthError / McpNotFoundError).
...
/** Default terminal classification: MCP auth (401/403) and not-found (404). */
 ...
 * treats McpAuthError (401/403) and McpNotFoundError (404) as terminal.
 *
 * McpRateLimitError honours `retryAfterMs` when present, otherwise falls
...
export async function withRetry<T>(
  ...
  const maxAttempts = opts.attempts ?? OPENAIRE_MCP_RETRY_ATTEMPTS
  const baseMs = opts.baseMs ?? OPENAIRE_MCP_RETRY_BASE_MS
  const capMs = opts.capMs ?? OPENAIRE_MCP_RETRY_CAP_MS
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      let waitMs: number
      if (err instanceof McpRateLimitError && err.retryAfterMs !== undefined) {
