...
//
// The OpenAIRE MCP (mcp-base) runs **stateless**: `initialize` returns no
// `mcp-session-id` header and each request is self-contained — so no session
// header is needed, and one must NOT be required. mcp-base servers previously
// ran stateful, but per-pod in-memory sessions broke once the HPA scaled them
// past one replica (no shared store / no session affinity → `400 No valid
// session ID provided` on every request that missed the owning pod). See the
// project memory `bnf-mcp-stateless-multireplica`.
//
...
import { McpAuthError, McpError } from "./errors"
 ...
 *
 * Throws McpAuthError on 401/403 (bad token — not retryable) and
 * McpError on any other transport failure.
 */
export async function openMcpSession(
  ...
  const res = await fetch(url, {
    ...
    signal: withTimeout(signal, OPENAIRE_MCP_TIMEOUT_MS),
  })

  if (res.status === 401 || res.status === 403) {
    throw new McpAuthError(`MCP initialize auth failed (HTTP ${res.status})`)
  }
  if (!res.ok) {
