// lib/constants.ts
// Cross-cutting string/number constants for the OpenAIRE Literature Research app.
...
export const ROUTES = {
  projects: "/projects",
  corpus: (projectId: string) => `/projects/${projectId}/corpus`,
  ingest: (projectId: string) => `/projects/${projectId}/ingest`,
  research: (projectId: string) => `/projects/${projectId}/research`,
  notebook: (projectId: string) => `/projects/${projectId}/research/notebook`,
 ...
 */
export const WORKSPACE_STEPS = ["corpus", "ingest", "research"] as const
...
// ---------------------------------------------------------------------------
// Layout geometry — prototype proportions (OpenAIRE Literature Research.dc.html).
...
/** Chat / workspace split on Step 1 — chat 40%, comprehension 60%. */
...
// for now; shown in the research workspace sub-header beside the doc count.
// (Prototype label: "cluster-oa-rag-01 · N docs".)
// ---------------------------------------------------------------------------

export const RAG_CLUSTER_ID = "cluster-oa-rag-01"

// ---------------------------------------------------------------------------
// Facet bar colors — the 7-hue dataset palette as CSS custom properties.
// Used by the corpus comprehension panel to color-code research-product types
// and distribution bars. Mapping mirrors the design prototype
// (OpenAIRE Literature Research.dc.html: articles indigo, preprints cyan,
// datasets pink, reviews green, software yellow, books orange).
// ---------------------------------------------------------------------------
...
/** Fixed type → dataset hue mapping from the prototype. Keys are the
 * `instanceType`-derived display types (lib/mcp/vocab.ts), lowercased. */
export const TYPE_DATASET_COLOR: Record<string, string> = {
  article: "var(--dataset-2)",
  preprint: "var(--dataset-3)",
  dataset: "var(--dataset-1)",
  review: "var(--dataset-4)",
  software: "var(--dataset-7)",
  book: "var(--dataset-6)",
  other: "var(--dataset-5)",
}
...
/**
 * Maximum ids echoed back in a remove-by-filter DRY RUN preview. The full
 ...
 * gate the operation. Generous headroom over the brief note the prompt asks
...
/**
 * MCP protocol version sent in the `initialize` handshake. The OpenAIRE MCP
 ...
 */
export const OPENAIRE_MCP_TIMEOUT_MS = 30_000

// ---------------------------------------------------------------------------
// Data-cluster MCP (real RAG) — see lib/cluster/datacluster-mcp-client.ts
// Same Streamable-HTTP / JSON-RPC contract as the OpenAIRE MCP (shared session
...
/** Slug prefix of a project's data-cluster dataset: `openaire-<projectId>`. */
export const DATACLUSTER_DATASET_SLUG_PREFIX = "openaire-"
...
/**
 * How many times the resolver retries a single id before flipping it to
...
/** Ids resolved per drainer batch (one MCP fan-out). */
 ...
 * resumes one, but a transient OpenAIRE outage leaves rows `pending` with no
 * further trigger — they would never recover without a new add or a restart.
 * This periodic sweep re-drains any project with pending stubs so resolution is
 * self-healing. Each pass increments resolveAttempts, so a genuinely-
 * unresolvable id still terminates at RESOLVE_MAX_ATTEMPTS. 3 min: prompt
 * recovery without hammering the Graph API.
 */
export const RESOLVE_SWEEP_INTERVAL_MS = 3 * 60 * 1_000

/**
 * Max in-flight MCP calls when resolving a batch of ids. The hosted
 * mcp-openaire fronts the public Graph API (rate-limited per IP); kept gentle.
 */
export const OPENAIRE_RESOLVE_CONCURRENCY = 4
...
export const OPENROUTER_APP_NAME = "Alien × OpenAIRE"
 ...
 * name lives in messages/{en,fr}.json. The first entry is the default.
 */
 ...
 * entry. The set of valid ids is AGENT_AVAILABLE_MODELS; this is the one
 * pre-selected when a session starts, and the model selector stays collapsed to
 * a bare icon while it is in effect. Distinct from AGENT_MODEL (the bare
 ...
 * message auto-names it. English-first, matching the default locale. */
...
export const FIRST_SESSION_TITLE = "First session"
 ...
 * 30 minutes: generous enough for a large corpus mutation MCP call but
 ...
 * against the OpenAIRE MCP / data-cluster MCP. Short so a dead server fails the
 * probe quickly rather than stalling the /api/health response. Tighter than the
 * full OPENAIRE_MCP_TIMEOUT_MS used for real tool calls — reachability is
 * binary and a healthy handshake returns in well under a second.
 */
 ...
 * for the Ingest page job history panel.
 */
export const INGEST_RECENT_JOBS_LIMIT = 20

// ---------------------------------------------------------------------------
// External URL templates — OpenAIRE / DOI
// ---------------------------------------------------------------------------
// Single source of truth for all external record links.
// Never construct these URLs inline — always use the functions below.
// See playbook/citations.md § "External URLs — derived only".

/** DOI resolver link ("Open via DOI" — resolves at the publisher).
 *  `doi` is the bare form ("10.1038/nbt.2647"), never a URL. */
export function DOI_URL(doi: string): string {
  return `https://doi.org/${doi}`
}

/**
 * OpenAIRE Explore record page ("OpenAIRE record" — full record in the Graph).
 * Prefers the PID (DOI) search form, which Explore resolves for every product
 * type; falls back to the OpenAIRE-id form for DOI-less products.
 */
export function OPENAIRE_RECORD_URL(openaireId: string, doi?: string | null): string {
  if (doi) {
    return `https://explore.openaire.eu/search/publication?pid=${encodeURIComponent(doi)}`
  }
  return `https://explore.openaire.eu/search/publication?articleId=${encodeURIComponent(openaireId)}`
}
