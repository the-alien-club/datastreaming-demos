...
// Overview card for the Ingestion step. Shows the current head version seq, the
// last ingested version seq, the delta (added / removed), and the CTA to start
// ingestion. Every resolved corpus document is ingestable (abstract + metadata;
// open-access full text where licensing permits), so there is no opt-in gate.
...
export function CardIngestSummary({
  ...
  const hasDelta = delta.added > 0 || delta.removed > 0
  const submitDisabled = isJobActive || isSubmitting || !hasDelta
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1 text-sm font-medium tabular-nums">
              ...
              <span
                className={
                  delta.removed > 0 ? "text-destructive" : "text-muted-foreground"
                }
              >
