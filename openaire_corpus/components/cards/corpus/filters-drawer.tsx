...
import { PRODUCT_TYPE, LANG } from "@/models/documents/schema"
...
function typeLabel(code: string): string {
  return PRODUCT_TYPE[code]?.label ?? code
}
function langLabel(code: string): string {
  return LANG[code]?.label ?? code
}
function oaLabel(code: string): string {
  return code
}

// Facet bar colors are dataset hues (dark-first). Type follows the prototype's
// fixed mapping; lang highlights the dominant language in brand teal; the OA
// bucket uses the fixed OA palette.
const OA_COLOR_VAR: Record<string, string> = {
  gold: "var(--dataset-7)",
  hybrid: "var(--dataset-6)",
  bronze: "var(--dataset-5)",
  green: "var(--dataset-4)",
...
function oaColorVar(code: string): string {
  return OA_COLOR_VAR[code] ?? "var(--dataset-5)"
}
...
export function CardCorpusFiltersDrawer({
  ...
  const oaSelected = csvToSelected(filters.oa)
  ...
  const activeCount =
    ...
    (filters.peer ? filters.peer.split(",").filter(Boolean).length : 0) +
    (filters.funder ? filters.funder.split(",").filter(Boolean).length : 0) +
  ...
  const hasAnyFacets =
    ...
    Object.keys(facets.oa).length > 0 ||
  ...
  const handleOaToggle = useCallback(
    (code: string) =>
      onChange({ ...filters, oa: toggleInCsv(filters.oa, code) }),
    [filters, onChange],
  )
      ...
      <CollapsibleContent className="rounded-md border border-t-0 bg-card px-4 py-4">
        <div className="flex flex-col gap-3.5">
          ...
          <div className="grid grid-cols-1 gap-3.5 md:grid-cols-3">
            ...
            {Object.keys(facets.oa).length > 0 && (
              <FacetCard title={t("facetOaTitle")}>
                <CardCorpusFacetBars
                  facet={facets.oa}
                  selected={oaSelected}
                  onToggle={handleOaToggle}
                  getLabel={oaLabel}
                  getColor={oaColorVar}
                  swatchShape="square"
                />
