export async function register() {
  ...
  // runs once, but a transient OpenAIRE outage strands rows in `pending` with no
  // further trigger — they would otherwise never recover without a new add or a
  // restart. This sweep re-drains any project with pending stubs so resolution
  // self-heals. Unlike the turn reaper (which must NOT run periodically — live
  // streaming turns are legitimate), pending stubs are never "in flight", so a
  // periodic sweep is safe.
  const { RESOLVE_SWEEP_INTERVAL_MS } = await import("@/lib/constants")
  setInterval(() => {
    void resumePendingResolves().catch((err: unknown) => {
