...
type Connectivity = { openaireMcpDown: boolean; dataclusterDown: boolean }
...
async function probeConnectivity(now: number): Promise<Connectivity> {
  ...
  let openaireMcpDown = false
  try {
    const env = requireMcpEnv()
    openaireMcpDown = !(await reachable(env.OPENAIRE_MCP_URL, env.OPENAIRE_MCP_TOKEN))
  } catch {
    openaireMcpDown = false
  }
  ...
  const value: Connectivity = { openaireMcpDown, dataclusterDown }
...
export class HealthService {
  ...
  static async snapshot(): Promise<HealthSnapshot> {
    ...
    const mcpUnreachable = conn.openaireMcpDown || conn.dataclusterDown
