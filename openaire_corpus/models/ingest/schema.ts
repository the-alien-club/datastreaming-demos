...
// The app-side coarse stages that the running ClusterProgressEvent maps onto.
// The worker's finer pipeline (resolve → fetchPdf → extract → prepare → embed →
// register) is grouped into these headline buckets for the progress UI:
//   dedup   ← resolve (delta + lane routing)
//   embed   ← fetchPdf + extract + prepare + embed
//   index   ← register
