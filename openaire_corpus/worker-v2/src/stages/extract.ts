/**
 * Extract stage — fulltext lane. Reads the downloaded PDF from S3, extracts
 * per-page text, and decides whether the PDF has a usable text layer:
 *   - median chars/page < ~100 → the PDF is scanned (image-only), no text layer.
 *     The doc DEGRADES to the abstract/metadata lane (re-emitted to prepare) — we
 *     don't OCR in this pipeline, so full text simply isn't available.
 *   - otherwise → emit a ResolvedDoc carrying `pageTexts` to prepare, which builds
 *     the page chunks.
 *
 * A corrupt/unreadable PDF (pdfjs throws) also degrades — the doc is never dropped.
 */
...
/** Median chars/page below this → treat as a scanned, no-text-layer PDF. */
...
export class ExtractStage extends PipelineStage<ResolvedDoc, ResolvedDoc> {
  ...
  async process(doc: ResolvedDoc, ctx: StageContext): Promise<StageOutcome<ResolvedDoc>> {
    ...
    let pages: string[];
    try {
      ({ pages } = await this.extractor.extract(bytes, { maxPages: this.maxPages }));
    } catch (e) {
    ...
    if (nonEmpty.length === 0 || med < MIN_MEDIAN_CHARS_PER_PAGE) {
      ctx.log.warn("extract_scanned_no_text_layer", { id: doc.openaireId, medianChars: med });
      await this.degrade(doc, "scanned_no_text_layer");
      return { kind: "done" };
    }

    ctx.log.info("extracted", { id: doc.openaireId, pages: pages.length, medianChars: med });
