...
/** The submit endpoint returns the created (or deduplicated) job view. */
export type SubmitIngestResult = IngestJobView
 ...
 *
 * Submission is asynchronous — the server returns the job row immediately
 * (track it with useIngestStatus) and deduplicates an in-flight job. Every
 * resolved corpus document is ingestable, so there is no confirmation gate.
 */
