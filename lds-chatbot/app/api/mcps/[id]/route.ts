...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const mcp = await db.query.mcps.findFirst({ where: (m, { eq }) => eq(m.id, id) })
...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  const { id } = await context.params
  const existing = await db.query.mcps.findFirst({ where: (m, { eq }) => eq(m.id, id) })
  ...
  const updated = await db.query.mcps.findFirst({ where: (m, { eq }) => eq(m.id, id) })
